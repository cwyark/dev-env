...
fish_add_path "$DEV_ENV_HOME/bin" "$HOME/.local/bin"
...
set -gx MISE_GLOBAL_CONFIG_FILE "$DEV_ENV_HOME/config/mise/config.toml"
set -gx MISE_DATA_DIR "$DEV_ENV_HOME/share/mise"
set -gx MISE_CACHE_DIR "$DEV_ENV_HOME/cache/mise"
set -gx MISE_STATE_DIR "$DEV_ENV_HOME/state/mise"

mkdir -p "$XDG_CONFIG_HOME" "$XDG_DATA_HOME" "$XDG_CACHE_HOME" "$XDG_STATE_HOME" "$MISE_DATA_DIR" "$MISE_CACHE_DIR" "$MISE_STATE_DIR"
...
if status is-interactive
    if test -d "$DEV_ENV_HOME/config/fish/completions"
        set -g fish_complete_path "$DEV_ENV_HOME/config/fish/completions" $fish_complete_path
    end
    ...
    if test (uname -s) = Darwin
        function docker --description "Run docker commands through Lima on macOS"
            ...
            if test -S "$lima_docker_socket"; and command -q docker
                ...
                printf 'dev-env: no Lima Docker socket or colima command found\n' >&2
                return 127
            end
        end
    end
